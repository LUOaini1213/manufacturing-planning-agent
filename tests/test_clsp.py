...
class TrigeiroTest(unittest.TestCase):
    def test_f1_highs_beats_a_capacity_blind_rule(self):
        ...
        self.assertLess(planned["objective"], 42680)
        self.assertLess(planned["objective"], merged["objective"])
        self.assertEqual(optimal["objective"], 40854.0)
        self.assertEqual(optimal["setups"], 46)
        self.assertIn("period 4", direct["reason"])
        self.assertFalse(shifted["ok"])
        self.assertIn("period 10", shifted["reason"])
        self.assertEqual(merged["objective"], 44519.0)
        self.assertEqual(merged["setups"], 51)
